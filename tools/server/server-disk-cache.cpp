...
static const uint32_t kIndexFormatVersion = 4; // bumped: embedded SHA-256 fix invalidates old hashes
...
bool server_disk_cache::load(const server_tokens& tokens, llama_context* ctx, int32_t slot_id,
    ...
    // Read the draft cache file (if any) before restoring anything: if this fails,
    // neither ctx nor ctx_dft has been touched yet, matching the "ordinary miss"
    // case the caller relies on (it leaves prompt.tokens stale on a miss, which is
    // only safe if the slot's physical KV memory was never mutated by this call).
    ...
    // Restore the draft context first: if this fails, ctx has not been touched yet.
    if (ctx_dft) {
    ...
    if (restored != data.size()) {
        ...
        if (ctx_dft) {
            // ctx_dft was already restored above but ctx failed: leave ctx_dft's
            // memory empty for this slot rather than an orphaned restored state.
            // Empty memory plus the caller's stale prompt.tokens correctly forces
            // a full reprocess (0 overlap), not a silent desync.
            llama_memory_seq_rm(llama_get_memory(ctx_dft), slot_id, -1, -1);
        }
        return false;
    }
...
bool server_disk_cache::save(const server_tokens& tokens, llama_context* ctx, int32_t slot_id,
    ...
    if (it != m_index.end()) {
        if (!ctx_dft || it->second.dft_size_bytes > 0) {
            // Already cached and this save has nothing new to contribute, just
        ...
        // Entry exists but was saved without draft-model state, and this save has
        // a draft context to contribute: a speculative-decoding server would
        // otherwise treat this hash as a permanent miss (see load()'s require_dft
        // handling) while last_used_us keeps getting refreshed, making it immune
        // to LRU eviction forever. Replace it with a full re-save instead.
        SRV_INF("disk cache: hash=%.8s... has no draft state, replacing with target+draft save\n", hash.c_str());
        ...
        if (it->second.dft_size_bytes > 0) {
            fs::remove(cache_path(m_path, hash + ".dft.bin"));
        }
        const uint64_t old_total = it->second.size_bytes + it->second.ckpt_size_bytes + it->second.media_size_bytes + it->second.dft_size_bytes;
...
bool server_disk_cache::load_index() {
    ...
    if (file_version != kIndexFormatVersion) {
        SRV_WRN("disk cache: index format version %u != %u (SHA-256 fix invalidates old hashes), clearing stale cache\n",
...
bool server_disk_cache::load_by_hash(const std::string & hash, llama_context * ctx, int32_t slot_id,
    ...
    // Read the draft cache file (if any) before restoring anything: see load()
    // for why order matters here - ctx must not be mutated before ctx_dft is
