...
def _spec_server(cache_dir):
    # Distinct target/draft models, mirroring test_speculative.py's proven-working
    # setup (target=stories15m_moe, draft=stories15M-q4_0.gguf) rather than
    # self-speculation, which this codebase does not support for this model (the
    # ctx_tgt_seq_rm_type check in server-context.cpp never enables the actual
    # speculative session when target and draft are the same model file here).
    s = ServerPreset.stories15m_moe()
    s.n_slots = 1
    s.n_predict = 8
    s.model_draft = download_file(SPEC_DRAFT_MODEL_URL)
...
def test_disk_cache_hit_with_speculative_decoding():
    ...
    Uses distinct target (stories15m_moe) and draft (stories15M-q4_0.gguf) models,
    matching test_speculative.py's proven working setup. With a single slot, an
    unrelated prompt reusing that slot forces the target and draft contexts to
    reprocess from scratch and settle at a small, real position. Requesting the
    original long prompt again then hits the entry saved by request A: ctx_tgt gets
    restored straight to its high saved position, while (pre-fix) ctx_dft is left
    at the short unrelated prompt's small position - a real, nonzero mismatch, which
    is exactly what triggered the production bug's "inconsistent sequence positions"
    / failed-decode loop.
    """
